...
struct scheduler_queue {
    ...
    using coro_handle = co::coroutine_handle<>;
...
struct prefetch_Awaitable {
    T* value_ = nullptr;
    prefetch_Awaitable() = default;
    prefetch_Awaitable(T& value) : value_(std::addressof(value)) {}
    prefetch_Awaitable& operator=(prefetch_Awaitable&& pa) = default;

    bool await_ready() { return false; }
    T await_resume() { return *value_; }
    template <typename Handle>
    auto await_suspend(Handle h) {
        _mm_prefetch(reinterpret_cast<char const*>(value_), _MM_HINT_NTA);
        auto& q = scheduler;
        q.push_back(h);
        return q.pop_front();
    }
...
struct root_task {
    struct promise_type;
    using HDL = co::coroutine_handle<promise_type>;
    ...
    struct promise_type {
        ...
        co::suspend_never final_suspend() { return {}; }
    };
